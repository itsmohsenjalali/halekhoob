...
def filtered_videos(request, home_page):
    ...
    if request.GET.get("mood", "").isdigit():
        selected = get_object_or_404(Mood, pk=request.GET["mood"], owner=request.user)
...
@login_required
def add(request):
    form = AddForm(request.POST or None, user=request.user)
            ...
            try:
                with transaction.atomic():
                    video, created = Video.objects.get_or_create(
                        source_key=key, owner=request.user,
                        defaults={
                            "owner": request.user,
                            "source_url": url,
                            "platform": platform,
...
def moods(request):
    instance = (
        get_object_or_404(Mood, pk=request.GET["edit"], owner=request.user)
