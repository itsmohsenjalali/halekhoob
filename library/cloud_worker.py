...
def publish(video, outputs, duration, source_title, lease):
    ...
    for path, kind, content_type in outputs:
        if path:
            key = f"{settings.R2_PREFIX}/media/{video.pk}/{attempt}/{path.name}"
            with archive_lock():
                lease.check(locked=True)
                record = CloudObject.objects.create(
                    key=key, video=video, size_bytes=path.stat().st_size, sha256=r2.checksum(path)
                )
...
def process_one(lease):
    ...
    with archive_lock(), transaction.atomic():
        ...
        video = (
            ...
            .filter(status="queued", next_attempt_at__lte=timezone.now())
